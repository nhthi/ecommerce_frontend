...
const ProductsTab = ({
  ...
  const topCategories = useMemo(
    () => (data?.topCategories ?? []).map((item) => ({ name: item.label, sales: item.value })),
    [data]
  );
    ...
    <Grid container spacing={2.2}>
      ...
      <Grid size={{ xs: 12, xl: 5 }}>
        <Paper elevation={0} sx={{ ...cardSx, p: 2.5, height: "100%" }}>
          <Typography sx={sectionTitleSx}>Danh mục bán tốt</Typography>
          ...
          <Box sx={{ mt: 2.2, height: 320 }}>
            {loading && topCategories.length === 0 ? (
              <Stack alignItems="center" justifyContent="center" sx={{ height: "100%" }} spacing={1.2}>
                ...
                <Typography sx={{ color: "#64748b" }}>Đang tải dữ liệu danh mục...</Typography>
              </Stack>
            ) : (
        ...
        <Paper elevation={0} sx={{ ...cardSx, p: 2.5, height: "100%" }}>
          ...
          <Box sx={{ mt: 2, height: 320 }}>
            {loading && inventoryData.length === 0 ? (
              <Stack alignItems="center" justifyContent="center" sx={{ height: "100%" }} spacing={1.2}>
                <CircularProgress sx={{ color: "#f97316" }} />
                <Typography sx={{ color: "#64748b" }}>Đang tải dữ liệu tồn kho...</Typography>
              </Stack>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={inventoryData}>
                  ...
                  <YAxis />
                  <Tooltip formatter={(value: number) => formatValue(value)} />
                  <Bar dataKey="inStock" fill="#16a34a" radius={[8, 8, 0, 0]} name="Con hang" />
                  <Bar dataKey="lowStock" fill="#f59e0b" radius={[8, 8, 0, 0]} name="Sap het" />
                  <Bar dataKey="outOfStock" fill="#ef4444" radius={[8, 8, 0, 0]} name="Het hang" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </Box>
          ...
          <Stack spacing={1.2} sx={{ mt: 2 }}>
            {loading && lowStockAlerts.length === 0 ? (
              <Stack alignItems="center" justifyContent="center" sx={{ py: 6 }} spacing={1.2}>
                <CircularProgress sx={{ color: "#f97316" }} />
                <Typography sx={{ color: "#64748b" }}>Đang tải cảnh báo tồn kho...</Typography>
              </Stack>
            ) : (
              lowStockAlerts.map((item) => (
                <Paper key={item.productId} elevation={0} sx={{ p: 1.5, borderRadius: "18px", backgroundColor: "#fffaf5", border: "1px solid rgba(249,115,22,0.14)" }}>
                  <Stack direction="row" justifyContent="space-between" spacing={2} alignItems="center">
                    <Box>
                      <Typography sx={{ color: "#0f172a", fontWeight: 700 }}>{item.productName}</Typography>
                      <Typography sx={{ mt: 0.35, color: "#64748b", fontSize: 13.5 }}>
                        SKU: {item.sku} · {item.categoryName}
                      </Typography>
                    </Box>
                    <Chip label={`Con ${item.quantity}`} sx={{ backgroundColor: "rgba(245,158,11,0.14)", color: "#b45309", fontWeight: 800 }} />
                  </Stack>
                </Paper>
              ))
            )}
            ...
            {!loading && lowStockAlerts.length === 0 && (
              <Paper elevation={0} sx={{ p: 2, borderRadius: "18px", backgroundColor: "#f8fafc", border: "1px solid rgba(15,23,42,0.06)" }}>
                <Typography sx={{ color: "#64748b", textAlign: "center" }}>
                  Khong co canh bao ton kho trong ky hien tai.
                </Typography>
              </Paper>
            )}
          </Stack>
