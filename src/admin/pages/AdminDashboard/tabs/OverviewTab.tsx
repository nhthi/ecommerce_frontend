...
import { danger, info, primary, success, violet } from "../dashboardData";
      ...
      {loading && !data ? (
        <Paper elevation={0} sx={{ ...cardSx, p: 5 }}>
          <Stack alignItems="center" spacing={1.4}>
            ...
            <Typography sx={{ color: "#64748b" }}>Đang tải dữ liệu tổng quan...</Typography>
          </Stack>
        </Paper>
      ) : (
        <>
                  ...
                  <Paper elevation={0} sx={{ ...cardSx, p: 2.3 }}>
                    <Stack direction="row" justifyContent="space-between">
                      <Box>
                        ...
                        <Typography sx={{ mt: 0.5, fontSize: 24, fontWeight: 900 }}>{item.value}</Typography>
                      </Box>
          ...
          <Grid container spacing={2.2}>
            ...
            <Grid size={{ xs: 12, xl: 4 }}>
              <Paper elevation={0} sx={{ ...cardSx, p: 2.5 }}>
                <Typography sx={sectionTitleSx}>
                  Top danh mục bán chạy {overviewFilter === "month" ? `tháng ${selectedMonth}/${selectedYear}` : `năm ${selectedYear}`}
                </Typography>
                <Box sx={{ height: 260 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
