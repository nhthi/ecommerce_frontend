        ...
        <div className="mx-auto flex max-w-[1560px] items-center justify-between gap-3 px-4 py-4 lg:px-8 xl:px-10">
          <div className="flex items-center gap-2 lg:gap-3">
            ...
            <button type="button" onClick={() => handleNavigate("/")} className="flex items-center gap-3 text-left">
              ...
              <div>
                <p className="text-[9px] font-bold uppercase tracking-[0.28em] text-orange-400">Thiết bị tập luyện</p>
                <h1 className="text-base font-black uppercase tracking-[0.18em] text-white md:text-xl xl:text-2xl">NHTHI FIT</h1>
              </div>
            </button>
          </div>
            ...
            {navItems.map((item) =>
              item.children ? (
                <div key={item.label} className="group relative flex h-full items-center py-6 -my-6">
                  <button type="button" className="flex items-center gap-0.5 px-2.5 text-[11px] font-bold uppercase tracking-[0.1em] text-white transition hover:text-orange-400 xl:px-3 xl:text-[12px]">
                    {item.label}
                  ...
                  <div className="invisible absolute left-0 top-full z-30 min-w-[250px] rounded-none border-t-2 border-orange-500 bg-[#111111] p-3 opacity-0 shadow-2xl shadow-black/50 transition duration-200 group-hover:visible group-hover:opacity-100">
                    {item.children.map((child) => (
                      <button key={child.label} type="button" onClick={() => handleNavigate(child.path)} className="flex w-full border-b border-white/5 px-3 py-3 text-left text-sm font-semibold text-slate-200 transition last:border-b-0 hover:bg-orange-500 hover:text-black">
                        {child.label}
                      </button>
                    ))}
                  </div>
                ...
                <button key={item.label} type="button" onClick={() => handleNavigate(item.path!)} className="px-2.5 text-[11px] font-bold uppercase tracking-[0.1em] text-white transition hover:text-orange-400 xl:px-3 xl:text-[12px]">
                  {item.label}
                </button>
              ),
            )}
          ...
          <div className="flex items-center gap-1 sm:gap-2 lg:gap-2">
            ...
            {auth?.user ? (
              <Button className="min-w-0 rounded-full" onClick={handleClickAvatar} sx={{ color: "white", textTransform: "none", pl: 0.75, pr: 1.2 }}>
      ...
      {mobileMenuOpen && (
        <div className="border-b border-orange-500/20 bg-[#0d0d0d] text-white lg:hidden">
          <div className="mx-auto flex max-w-[1560px] flex-col gap-2 px-4 py-4">
            {!auth?.user && (
              <button type="button" onClick={() => handleNavigate("/login")} className="border-b border-white/10 py-3 text-left text-sm font-bold uppercase tracking-[0.16em] text-white">
                Tài khoản
              </button>
            )}
            {navItems.map((item) =>
              item.children ? (
                <div key={item.label} className="border-b border-white/10 pb-2">
                  <button type="button" onClick={() => toggleMobileSection(item.label)} className="flex w-full items-center justify-between py-3 text-left text-sm font-bold uppercase tracking-[0.16em] text-white">
                    {item.label}
                  ...
                  {expandedMobileMenu === item.label && (
                    <div className="flex flex-col bg-white/5 px-2 py-2">
                      {item.children.map((child) => (
                        <button key={child.label} type="button" onClick={() => handleNavigate(child.path)} className="px-3 py-3 text-left text-sm font-medium text-slate-100 transition hover:bg-orange-500 hover:text-black">
                          {child.label}
                        </button>
                      ))}
                    </div>
                ...
                <button key={item.label} type="button" onClick={() => handleNavigate(item.path!)} className="border-b border-white/10 py-3 text-left text-sm font-bold uppercase tracking-[0.16em] text-white">
                  {item.label}
                </button>
              ),
            )}
