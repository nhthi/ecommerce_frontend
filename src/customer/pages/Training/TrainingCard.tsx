...
const TrainingCard = ({ item }: { item: TrainingSchedule }) => {
  ...
  return (
    <Paper elevation={0} sx={{ overflow: "hidden", borderRadius: "28px", border: "1px solid rgba(255,255,255,0.08)", background: "linear-gradient(180deg, rgba(20,20,20,0.98), rgba(10,10,10,0.99))", boxShadow: "0 24px 60px rgba(0,0,0,0.28)", color: "white", transition: "transform 0.22s ease, border-color 0.22s ease", "&:hover": { transform: "translateY(-4px)", borderColor: "rgba(249,115,22,0.26)" } }}>
      ...
      <Box sx={{ p: 2.2 }}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1.2} flexWrap="wrap">
          <Chip icon={<CalendarMonth sx={{ color: "#fb923c !important" }} />} label={`${item.durationWeeks} tuan`} variant="outlined" sx={{ color: "#fff7ed", borderColor: "rgba(249,115,22,0.22)" }} />
          <Chip icon={<AccessTime sx={{ color: "#fb923c !important" }} />} label={`${item.sessionsPerWeek} buoi / tuan`} variant="outlined" sx={{ color: "#fff7ed", borderColor: "rgba(249,115,22,0.22)" }} />
          <Chip icon={<TrendingUp sx={{ color: "#fb923c !important" }} />} label={item.sessionLength} variant="outlined" sx={{ color: "#fff7ed", borderColor: "rgba(249,115,22,0.22)" }} />
        </Stack>

        <Stack spacing={1} sx={{ mt: 2 }}>
          <Stack direction="row" spacing={1} alignItems="center">
            <PersonOutline sx={{ fontSize: 18, color: "#fb923c" }} />
            <Typography sx={{ color: "rgba(255,255,255,0.76)", fontSize: 14 }}>{item.coach}</Typography>
          </Stack>

          <Stack direction="row" spacing={1} alignItems="flex-start">
            <FitnessCenter sx={{ fontSize: 18, color: "#fb923c", mt: 0.2 }} />
            <Typography sx={{ color: "rgba(255,255,255,0.76)", fontSize: 14 }}>{item.equipment.join(" • ")}</Typography>
          </Stack>
        </Stack>

        <Box sx={{ mt: 2.2, borderRadius: "20px", border: "1px solid rgba(255,255,255,0.08)", backgroundColor: "rgba(255,255,255,0.03)", p: 1.6 }}>
          <Typography fontWeight={700} sx={{ color: "#fff", mb: 1.1 }}>Lich tap mau</Typography>
          <Stack spacing={0.8}>
            {item.schedule.map((line) => (
              <Typography key={line} sx={{ color: "rgba(255,255,255,0.7)", fontSize: 13.5 }}>{line}</Typography>
            ))}
          </Stack>
        </Box>
